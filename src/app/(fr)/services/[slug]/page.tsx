...
export function generateStaticParams() {
  return services.map((s) => ({ slug: s.slug.fr }));
}
...
export async function generateMetadata({
  ...
  const service = findServiceBySlug("fr", slug);
  if (!service) return {};
  return {
    title: service.fr.seo.title,
    description: service.fr.seo.description,
    alternates: {
      languages: {
...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <ServicePageView service={service} locale="fr" />;
}
